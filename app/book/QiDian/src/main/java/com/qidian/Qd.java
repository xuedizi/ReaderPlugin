 ...
 * @created 2018/12/17
 */
public class Qd implements IBookSupport {
  ...
  @Override public JSONObject getNovelInfoByUrl(String url) {
    ...
    try {
      ...
      resultObject.put(BK.Novel_logo, QdUrl.Url_Base_Http + novelLogo);
    ...
    try {
      ...
      for (int array = 0; array < vsArray.length(); array++) {
        ...
        for (int index = 0; index < cs.length(); index++) {
          ...
          try {
            if (isFree) {
              jsonObject.put(BK.Catalogue_Path, QdUrl.Url_Chapter + csJSONObject.optString("cU"));
            }
            jsonObject.put(BK.Catalogue_Title, csJSONObject.optString("cN"));
  ...
  @Override public JSONArray getNavigateList() {
    ...
    if (null != nodeList) {
      ...
      for (int index = 0; index < nodeList.getLength(); index++) {
        ...
        JSONObject jsonObject = new JSONObject();
        try {
          jsonObject.put(BK.Navigate_Name, name).put(BK.Navigate_Path, url);
          jsonArray.put(jsonObject);
        } catch (JSONException e) {
          e.printStackTrace();
        }
    ...
    if (null != nodeList) {
      ...
      for (int index = 0; index < nodeList.getLength(); index++) {
        ...
        try {
          ...
          jsonObject.put(BK.Novel_intro, intro);
