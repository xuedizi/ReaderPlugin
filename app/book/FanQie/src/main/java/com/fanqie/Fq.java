 ...
 * @created 2019/1/15
 */
public class Fq implements IBookSupport {
  ...
  private JSONObject getNovelInfo(Document dom, String url) {
    ...
    try {
      jsonObject.put(BK.Novel_Path, url);
      jsonObject.put(BK.Novel_Name, novelName);
      jsonObject.put(BK.Novel_Author, novelAuthor);
      jsonObject.put(BK.Novel_intro, novelIntro);
      jsonObject.put(BK.Novel_last_chapter, novelLastChapter);
      jsonObject.put(BK.Novel_last_modified, lastModified);
      jsonObject.put(BK.Novel_logo, novelLogo);
      jsonObject.put(BK.Novel_Catalogue, chapters);
  ...
  public JSONArray getChapters(Document dom) {
    ...
    for (int index = 0; index < len; index++) {
      JSONObject jsonObject = new JSONObject();
      try {
        jsonObject.put(BK.Catalogue_Path, FqUrl.Url_Base_Book + urls.get(index));
        jsonObject.put(BK.Catalogue_Title, titles.get(index));
  ...
  @Override public JSONObject getChapterContent(String url) {
    ...
    try {
      ...
      jsonObject.put(BK.Chapter_Path, url);
    ...
    for (int index = 0; index < nodeList.getLength(); index++) {
      ...
      try {
        jsonArray.put(new JSONObject().put(BK.Novel_Path, path)
            .put(BK.Novel_last_modified, lastModifiedL)
            .put(BK.Novel_Name, name)
            .put(BK.Novel_last_chapter, lastChapter)
  ...
  private JSONArray getLasterUpdateItem(Document dom) {
    ...
    if (null != nodeList) {
      ...
      for (int index = 0; index < nodeList.getLength(); index++) {
        ...
        JSONObject jsonObject = new JSONObject();
        try {
          jsonObject.put(BK.Novel_Path, FqUrl.Url_Base + path);
