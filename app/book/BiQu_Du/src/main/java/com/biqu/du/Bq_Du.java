 ...
 * @created 2018/12/17
 */
public class Bq_Du implements IBookSupport {
  ...
  public JSONObject getNovelInfo(Document dom, String url) {
    ...
    List<String> chaptersTitle = XpathHelper.getN(dom, Bq_DuXpathUri.Xpath_chapter_list);
    List<String> chaptersUrl = XpathHelper.getN(dom, Bq_DuXpathUri.Xpath_chapter_url);
    if (null == chaptersTitle || null == chaptersUrl) {
      return null;
    }
    if (chaptersTitle.size() > 12 && chaptersUrl.size() > 12) {
      for (int i = 0; i < 12; i++) {
        chaptersTitle.remove(0);
        chaptersUrl.remove(0);
      }
    }
    String novelName = XpathHelper.get(dom, Bq_DuXpathUri.Xpath_novel_name);
    //获取作者名称
    String novelAuthor = null;
    List<String> ps = XpathHelper.getN(dom, Bq_DuXpathUri.Xpath_novel_author);
    if (null != ps && !ps.isEmpty() && ps.get(0).split("：").length > 1) {
      novelAuthor = ps.get(0).split("：")[1];
    }
    //获取最后更新时间
    String lastModifiedStr = XpathHelper.get(dom, Bq_DuXpathUri.Xpath_novel_last_modified);
    long lastModified = getLastModified(lastModifiedStr);
    String novelLogo = Bq_DuUrl.Url_Base + XpathHelper.get(dom, Bq_DuXpathUri.Xpath_novel_logo);
    String novelIntroduction = XpathHelper.get(dom, Bq_DuXpathUri.Xpath_novel_introduction);
    ...
    for (int index = 0; index < len; index++) {
      ...
      try {
        jsonObject.put(BK.Catalogue_Path, Bq_DuUrl.Url_Base + chaptersUrl.get(index));
    ...
    //获取最新章节
    String newestChapter = XpathHelper.get(dom, Bq_DuXpathUri.Xpath_novel_last_chapter);
    try {
  ...
  @Override public JSONObject getNovelInfoByName(String novelname) {
    ...
    Document dom = XpathHelper.getDom(String.format(Bq_DuUrl.Url_Search, novelnameEdcode));
    String url = null;
    //判断是搜索列表页或小说结果页
    List<String> titles = XpathHelper.getN(dom, Bq_DuXpathUri.Xpath_Novel_Search_Title_List);
    List<String> urls = XpathHelper.getN(dom, Bq_DuXpathUri.Xpath_Novel_Search_Url_List);

    if (null != titles && !titles.isEmpty()) {
      //搜索结果列表类型
      if (titles.contains(novelname)) {
        if (null != urls) {
          url = Bq_DuUrl.Url_Base + urls.get(titles.indexOf(novelname));
          dom = XpathHelper.getDom(url);
        }
      }
    } else {
      //搜索结果小说详情
      String title = XpathHelper.get(dom, Bq_DuXpathUri.Xpath_novel_name);
      if (!TextUtils.isEmpty(title)) {
        String chapterUrl = XpathHelper.get(dom, Bq_DuXpathUri.Xpath_chapter_url);
        assert chapterUrl != null;
        url = Bq_DuUrl.Url_Base + chapterUrl.substring(0, chapterUrl.lastIndexOf("/"));
      }
    }
    //没有获取到链接直接返回
  ...
  @Override public JSONObject getChapterContent(String chapterUrl) {
    ...
    String title = XpathHelper.get(dom, Bq_DuXpathUri.Xpath_chapter_title);
    List<String> list = XpathHelper.getN(dom, Bq_DuXpathUri.Xpath_chapter_content);
  ...
  @Override public JSONArray getNovelInfoByAuthor(String author) {
    Document dom = XpathHelper.getDom(String.format(Bq_DuUrl.Url_Search, author));
    NodeList nodeList = XpathHelper.getNodeList(dom, Bq_DuXpathUri.Xpath_Search_Item);
    if (null == nodeList) return null;
    JSONArray jsonArray = new JSONArray();
    for (int index = 0; index < nodeList.getLength(); index++) {
      Node item = nodeList.item(index);
      String path = Bq_DuUrl.Url_Base + XpathHelper.get(item, Bq_DuXpathUri.Xpath_Search_Path);
      String name = XpathHelper.get(item, Bq_DuXpathUri.Xpath_Search_Name);
      String logo = Bq_DuUrl.Url_Base + XpathHelper.get(item, Bq_DuXpathUri.Xpath_Search_Logo);
      String authorName = XpathHelper.get(item, Bq_DuXpathUri.Xpath_Search_Author);
      String intro = XpathHelper.get(item, Bq_DuXpathUri.Xpath_Search_Intro);
      try {
  ...
  @Override public JSONArray getNavigateList() {
    Document dom = XpathHelper.getDom(Bq_DuUrl.Url_Base);
    List<String> navUrls = XpathHelper.getN(dom, Bq_DuXpathUri.Xpath_Navigate_Url_List);
    List<String> navNames = XpathHelper.getN(dom, Bq_DuXpathUri.Xpath_Navigate_Name_List);
    ...
    for (int index = 0; index < navUrls.size(); index++) {
      ...
      try {
        ...
        if (null != path && !path.startsWith(Bq_DuUrl.Url_Base)) {
          path = Bq_DuUrl.Url_Base + path;
        }
  ...
  private JSONArray getLasterUpdateItem(Document dom) {
    NodeList nodeList =
        XpathHelper.getNodeList(dom, Bq_DuXpathUri.Xpath_Navigate_Laster_Update_Novel_Item);
    if (null != nodeList) {
      JSONArray jsonArray = new JSONArray();
      for (int index = 0; index < nodeList.getLength(); index++) {
        Node node = nodeList.item(index);
        String path = XpathHelper.get(node, Bq_DuXpathUri.Xpath_Navigate_Laster_Update_Novel_Path);
        String author =
            XpathHelper.get(node, Bq_DuXpathUri.Xpath_Navigate_Laster_Update_Novel_Author);
        String name = XpathHelper.get(node, Bq_DuXpathUri.Xpath_Navigate_Laster_Update_Novel_Name);
        String lastChapter =
            XpathHelper.get(node, Bq_DuXpathUri.Xpath_Navigate_Laster_Update_Novel_Laster_Chapter);
        JSONObject jsonObject = new JSONObject();
        try {
          jsonObject.put(BK.Novel_Path, Bq_DuUrl.Url_Base + path);
  ...
  public JSONArray getNavigateItem(Document dom) {
    NodeList nodeList =
        XpathHelper.getNodeList(dom, Bq_DuXpathUri.Xpath_Navigate_Novel_Item);
    if (null == nodeList) return null;
    JSONArray novels = new JSONArray();
    for (int m = 0; m < nodeList.getLength(); m++) {
      Node item = nodeList.item(m);
      List<String> introList = XpathHelper.getN(item, Bq_DuXpathUri.Xpath_Navigate_Novel_Intro);
      String intro = null;
      if (null != introList) {
        StringBuilder buffer = new StringBuilder();
      ...
      String author = XpathHelper.get(item, Bq_DuXpathUri.Xpath_Navigate_Novel_Author);
      String logo = XpathHelper.get(item, Bq_DuXpathUri.Xpath_Navigate_Novel_Logo);
      if (null != logo && !logo.startsWith(Bq_DuUrl.Url_Base)) {
        logo = Bq_DuUrl.Url_Base + logo;
      }
      String path = XpathHelper.get(item, Bq_DuXpathUri.Xpath_Navigate_Novel_Path);
      if (null != path && !path.startsWith(Bq_DuUrl.Url_Base)) {
        path = Bq_DuUrl.Url_Base + path;
      }
      String name = XpathHelper.get(item, Bq_DuXpathUri.Xpath_Navigate_Novel_Name);
  ...
  public static void main(String args[]) {
    String allChapterUrls = new Bq_Du().getNovelInfoByName("武动乾坤").toString();
