 ...
 * @created 2018/12/17
 */
public class Fy implements IBookSupport {
  ...
  public JSONObject getNovelInfo(Document domcument, String url) {
    ...
    for (int index = 0; index < len; index++) {
      ...
      try {
        jsonObject.put(BK.Catalogue_Path, Url.Url_Base + chaptersUrl.get(index));
    ...
    String newestChapter =
        jsonArray.optJSONObject(jsonArray.length() - 1).optString(BK.Catalogue_Title);
    try {
      resultObject.put(BK.Novel_Name, trim(novelName));
      resultObject.put(BK.Novel_Path, url);
      resultObject.put(BK.Novel_Catalogue, jsonArray);
      resultObject.put(BK.Novel_Author, null);
      resultObject.put(BK.Novel_last_chapter, newestChapter);
      resultObject.put(BK.Novel_logo, null);
      resultObject.put(BK.Novel_intro, null);
    } catch (JSONException e) {
      e.printStackTrace();
      return null;
    }
  ...
  @Override public JSONObject getChapterContent(String chapterUrl) {
    ...
    try {
      ...
      jsonObject.put(BK.Chapter_Path, Url.Url_Base + chapterUrl);
  ...
  private JSONArray getLasterUpdateItem(Document dom) {
    ...
    if (null != nodeList) {
      ...
      for (int index = 0; index < nodeList.getLength(); index++) {
        ...
        JSONObject jsonObject = new JSONObject();
        try {
          jsonObject.put(BK.Novel_Path, Url.Url_Base + path);
